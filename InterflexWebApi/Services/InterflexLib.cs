...
namespace InterflexWebApi.Services
{
    public class InterflexLib : IInterflexLib
    {
        const string URI = "https://interflex.kistler.com:8443/WebClient/iflx/login.jsp?userlocale=en";
        private static IWebDriver _driver;

        public async Task<List<DayAndTime>> GetInterflexData(string username, string password)
        {
            ...
            try
            {
                ...
                //const string remoteUrl = "http://chrome-node:4444/wd/hub";
                const string remoteUrl = "http://selenium-hub:4444/wd/hub";
                _driver = new RemoteWebDriver(new Uri(remoteUrl), options);
                Console.WriteLine($"Running on remote driver");
